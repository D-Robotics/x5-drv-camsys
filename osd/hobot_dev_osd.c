...
static int osd_start_worker(struct x3_osd_dev *osd_dev)
{
	...
	if (ret) {
		mutex_unlock(&osd_dev->osd_mutex);
		vio_err("sched_setattr_nocheck is fail(%d)", ret);
		goto exit;
	}
...
static void osd_frame_process(struct vio_osd_info *osd_info, struct vio_frame *frame)
{
	...
	// }
	osd_subdev = &osd_dev->subdev[osd_info->inst_id][osd_info->chn_id];
	...
	// }
	if (frame != NULL) {
		osal_spin_lock_irqsave(&osd_subdev->frame_slock, &flags);
		osd_put_input_frame(&osd_subdev->input_frame_list, frame);
		osal_spin_unlock_irqrestore(&osd_subdev->frame_slock, &flags);

		atomic_inc(&osd_subdev->osd_info->frame_count);
		kthread_queue_work(&osd_dev->worker, &osd_subdev->work);
	} else {
		vio_warn("vse call osd, but frame was null\n");
		goto exit;
	}
	return;
...
static void osd_process_workfunc_done(struct osd_subdev *subdev)
{
	...
	// struct pym_subdev *pym_subdev;
	struct x3_osd_dev *osd_dev;
	int32_t instance, chn_id;
	osal_time_t tmp_tv;

	osd_dev = subdev->osd_dev;
	// if (subdev->chn_id == OSD_PYM_OUT) {
	...
	instance = subdev->osd_info->inst_id;
	chn_id = subdev->osd_info->chn_id;

	osal_time_get(&tmp_tv);
	g_osd_idx[instance][chn_id]++;
	if (tmp_tv.tv_sec > g_osd_fps_lasttime[instance][chn_id]) {
		g_osd_fps[instance][chn_id] = g_osd_idx[instance][chn_id];
		g_osd_fps_lasttime[instance][chn_id] = tmp_tv.tv_sec;
		g_osd_idx[instance][chn_id] = 0;
	}

	if (atomic_dec_return(&subdev->osd_info->frame_count) > 0) {
		kthread_queue_work(&osd_dev->worker, &subdev->work);
	}
	vio_dbg("[S%d][V%d] %s\n", instance, chn_id, __func__);
}
...
static void osd_frame_process_workfunc(struct kthread_work *work)
{
		...
		if (subdev->chn_id >= OSD_IPU_DS2) {
			...
			for (i = 0; i < MAX_STA_NUM; i++) {
				...
				if (subdev->osd_sta.sta_box[i].sta_en) {
					...
					process_info->frame_id = frame->common_frame.frameinfo.frameid.frame_id;
					process_info->buffer_index = frame->common_frame.frameinfo.bufferindex;
					if (subdev->chn_id == OSD_PYM_OUT) {
					// osd_process_set_pym_addr(process_info, frame, pym_subdev);
					} else {
						process_info->image_width = frame->common_frame.vbuf.group_info.info[0].buf_attr.width;
						process_info->image_height = frame->common_frame.vbuf.group_info.info[0].buf_attr.height;
						process_info->tar_y_addr = __va(frame->common_frame.vbuf.group_info.info[0].paddr[0]);
						process_info->tar_uv_addr = __va(frame->common_frame.vbuf.group_info.info[0].paddr[0]);
					}
	...
	for (i = 0; i < OSD_LEVEL_NUM; i++) {
		...
		list_for_each_entry_safe(bind, temp, &subdev->bind_list, node) {
			...
			bind->proc_info.frame_id = frame->common_frame.frameinfo.frameid.frame_id;
			...
			if (subdev->chn_id == OSD_PYM_OUT) {
				...
				bind->proc_info.image_width = frame->common_frame.vbuf.group_info.info[0].buf_attr.width;
				bind->proc_info.image_height = frame->common_frame.vbuf.group_info.info[0].buf_attr.height;
				bind->proc_info.tar_y_addr = __va(frame->common_frame.vbuf.group_info.info[0].paddr[0]);
				bind->proc_info.tar_uv_addr = __va(frame->common_frame.vbuf.group_info.info[0].paddr[0]);
			}
...
static int osd_create_handle(struct osd_video_ctx *osd_ctx, unsigned long arg)
{
	...
	ret = (int32_t)copy_from_user((void *)&handle->info, (void __user *)arg, sizeof(osd_handle_info_t));
	if (ret) {
...
static int osd_get_attr(struct osd_video_ctx *osd_ctx, unsigned long arg)
{
	...
	ret = (int32_t)copy_from_user((void *)&handle_info, (void __user *)arg, sizeof(osd_handle_info_t));
	if (ret) {
		vio_err("%s: copy_from_user failed (ret=%d)\n", __func__, ret);
		ret = -EFAULT;
		goto exit;
	...
	ret = (int32_t)copy_to_user((void __user *)arg, (void *)&handle->info, sizeof(osd_handle_info_t));
	mutex_unlock(&osd_dev->osd_list_mutex);
...
static int osd_get_buffer(struct osd_video_ctx *osd_ctx, unsigned long arg)
{
	...
	ret = (int32_t)copy_from_user((void *)&buffer_info, (void __user *)arg, sizeof(osd_buffer_info_t));
	if (ret) {
		vio_err("%s: copy_from_user failed (ret=%d)\n", __func__, ret);
		ret = -EFAULT;
		goto exit;
	...
	ret = (int32_t)copy_to_user((void __user *)arg, (void *)&buffer_info, sizeof(osd_buffer_info_t));
...
static int osd_attach(struct osd_video_ctx *osd_ctx, unsigned long arg)
{
	...
	ret = (int32_t)copy_from_user((void *)&bind->bind_info, (void __user *)arg, sizeof(osd_bind_info_t));
	if (ret) {
		vio_err("%s: copy_from_user failed (ret=%d)\n", __func__, ret);
		ret = -EFAULT;
		goto exit_free_bind;
...
static int osd_detach(struct osd_video_ctx *osd_ctx, unsigned long arg)
{
	...
	ret = (int32_t)copy_from_user((void *)&bind_info, (void __user *)arg, sizeof(osd_bind_info_t));
	if (ret) {
		vio_err("%s: copy_from_user failed (ret=%d)\n", __func__, ret);
		ret = -EFAULT;
		goto exit;
...
static int osd_get_bind_attr(struct osd_video_ctx *osd_ctx, unsigned long arg)
{
	...
	ret = (int32_t)copy_to_user((void __user *)arg, (void *)&bind->bind_info, sizeof(osd_bind_info_t));
	mutex_unlock(&subdev->bind_mutex);
...
static int osd_set_sta(struct osd_video_ctx *osd_ctx, unsigned long arg)
{
	...
	ret = (int32_t)copy_from_user((void *)&sta_info, (void __user *)arg, sizeof(osd_sta_info_t));
	if (ret) {
		vio_err("%s: copy_from_user failed (ret=%d)\n", __func__, ret);
		ret = -EFAULT;
		goto exit;
	...
	memcpy(subdev->osd_sta.sta_box, sta_info.sta_box, MAX_STA_NUM * sizeof(osd_sta_box_t));
	subdev->osd_sta.sta_state = OSD_STA_REQUEST;
	if (subdev->chn_id <= OSD_IPU_DS1) {
...
static int osd_get_sta_bin_value(struct osd_video_ctx *osd_ctx, unsigned long arg)
{
	...
	ret = (int32_t)copy_from_user((void *)&sta_bin_info, (void __user *)arg, sizeof(osd_sta_bin_info_t));
	if (ret) {
		vio_err("%s: copy_from_user failed (ret=%d)\n", __func__, ret);
		ret = -EFAULT;
		goto exit;
	...
	ret = (int32_t)copy_to_user((void __user *)arg, (void *)&sta_bin_info, sizeof(osd_sta_bin_info_t));
...
static int osd_set_color_map(struct osd_video_ctx *osd_ctx, unsigned long arg)
{
	...
	ret = (int32_t)copy_from_user((void *)&osd_color, (void __user *)arg, sizeof(osd_color_map_t));
	if (ret) {
		vio_err("%s: copy_from_user failed (ret=%d)\n", __func__, ret);
		ret = -EFAULT;
		goto exit;
...
static int osd_proc_buf(struct osd_video_ctx *osd_ctx, unsigned long arg)
{
	...
	ret = (int32_t)copy_from_user((void *)&proc_buf, (void __user *)arg, sizeof(osd_proc_buf_info_t));
	if (ret) {
		vio_err("%s: copy_from_user failed (ret=%d)\n", __func__, ret);
		ret = -EFAULT;
		goto exit;
...
void x3_osd_subdev_init(struct x3_osd_dev *osd)
{
	...
	for (i = 0; i < VIO_MAX_STREAM; i++) {
		for (j = 0; j < OSD_CHN_MAX; j++) {
			...
			subdev->chn_id = j;
			subdev->inst_id = i;
			// todo: need get osd info addr and osd_cfg from vse
...
static int x3_osd_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		vio_err("osd cops register failed\n");
		return ret;
	}
