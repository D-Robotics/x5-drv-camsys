...
int32_t common_init(uint8_t chn, uint8_t mode)
{
	...
	get_common_info(chn);
	get_tuning_info(chn);
	// just give exptime a init value to prevent that: user did not config this value
	// maybe the value 400 is not suitable for all sensor
	if (sensor_param[chn].exposure_time_init == 0) {
		sensor_param[chn].exposure_time_init = 400;
	}
