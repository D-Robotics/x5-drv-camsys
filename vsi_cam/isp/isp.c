...
static int isp_set_stream_buffer(struct isp_device *isp, u32 inst)
{
	struct isp_instance *ins;
	...
	if (!has_offline(ctx->src_online_stat)) {
		...
		if (!ctx->src_buf && !ctx->next_src_buf) {
			for (i = 0; i < 2; i++) {
				rc = new_frame(ctx);
				if (rc) {
					rc = -ENOMEM;
			...
			isp_set_mp_buffer(isp, get_phys_addr(isp->cam_dev, ctx->src_buf, 0),
					  &ins->fmt.ofmt);
			isp_mp_buffer_force_update(isp);
			isp_set_schedule(isp, &sch, 0, 0, false);
		}
...
int isp_set_state(struct isp_device *isp, u32 inst, int state, enum group_type type)
{
	...
	if (!rc && state == CAM_STATE_STARTED) {
		if (isp->mode != ISP_STRM_MODE) {
			...
			if (has_offline(ctx->src_online_stat)) {
				if (ctx->next_src_buf)
					isp_set_mp_buffer(isp, get_phys_addr(isp->cam_dev, ctx->next_src_buf, 0),
							  &ins->fmt.ofmt);
			} else {
				isp_set_mp_buffer(isp, 0, &ins->fmt.ofmt);
			}
...
static int isp_set_schedule_online_stream(struct isp_device *isp, bool isp_irq_call)
{
	...
	if (has_offline(ctx->src_online_stat)) {
		...
		isp_set_mp_buffer(isp, get_phys_addr(isp->cam_dev, node->data, 0), &ins->fmt.ofmt);
	} else {
		isp_set_mp_buffer(isp, 0, &ins->fmt.ofmt);
	}
...
static int isp_set_schedule_online_mcm(struct isp_device *isp, struct isp_mcm_sch *sch)
{
	...
	if (mcm_ib) {
		if (has_offline(ctx->src_online_stat)) {
			...
			sch->mp_buf.mem.addr = get_phys_addr(isp->cam_dev, node->data, 0);
					...
					bool isp_irq_call)
{
	...
	if (ctx->sink_buf) {
		...
		sch->mp_buf.valid = 1;
		sch->rdma_buf.mem.addr = get_phys_addr(isp->cam_dev, ctx->sink_buf, 0);
